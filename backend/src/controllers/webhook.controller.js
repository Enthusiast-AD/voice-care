...
export const handleUltravoxWebhook = asyncHandler(async (req, res) => {
    ...
    const { call_id, patient_id, transcript, extracted_symptoms, risk_score } = req.body;

    if (!call_id || !patient_id) {
        throw new ApiError(400, "Missing required webhook parameters: call_id or patient_id");
    }

    console.log(`[Webhook] Received call data for Patient: ${patient_id}, Risk: ${risk_score}`);

    // 2. Map risk score to enum/string
    // Usually Low/Medium/High/Critical as per PRD
    const finalRiskScore = risk_score || 'Low';

    // 3. Save to Supabase (Call Logs table)
    ...
    if (dbError) {
        console.error("Ultravox DB Error:", dbError);
        throw new ApiError(500, "Failed to log call data into Supabase", [dbError.message]);
    }
    ...
    if (finalRiskScore === 'High' || finalRiskScore === 'Critical') {
        console.log(`🚨 ALERT! Patient ${patient_id} has a ${finalRiskScore} risk level. Dispatching SMS to Doctor.`);
    ...
    return res.status(200).json(
        new ApiResponse(200, callLog, "Webhook processed and logged successfully")
    );
});
